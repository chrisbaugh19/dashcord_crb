            ...
            if (!empty($sumColumns) || !empty($avgColumns) || !empty($maxColumns) || !empty($minColumns)) {
                ...
                if (!empty($sumColumns) || !empty($avgColumns)) {
                    ...
                    foreach ($output['sumAvgColumns'] as $key => $columnTitle) {
                        if (isset($sumColumns[$key]) && strpos($sumColumns[$key], 'formula') !== false){
                            ...
                            array_unshift($functionsParsedQuery['SELECT'], array(
                                    'expr_type' => 'colref',
                                    'base_expr' => 'SUM(' . $wdtParameters['columnFormulas'][$formulaColumnTitle] . ')
                                                 AS ' . $leftSysIdentifier . $columnTitle . $rightSysIdentifier,
                                    'delim' => ','
                                )
                            );
                        } else if( isset($avgColumns[$key]) && strpos($avgColumns[$key], 'formula') !== false){
                            $formulaColumnTitle = $avgColumns[$key];
                            if(strpos($wdtParameters['columnFormulas'][$formulaColumnTitle], 'sec(') !== false){
                                $wdtParameters['columnFormulas'][$formulaColumnTitle] = str_replace('sec(','1/cos(',$wdtParameters['columnFormulas'][$formulaColumnTitle]);
                            }
                            if(strpos($wdtParameters['columnFormulas'][$formulaColumnTitle], 'csc(') !== false){
                    ...
                    foreach ($this->getAvgColumns() as $columnTitle) {
                        ...
                        foreach ($main_res_dataRows as $row) {
                            if (strpos($columnTitle, 'formula') !== false) {
                                foreach ($aColumns as $aCol) {
                                    if (strpos($wdtParameters['columnFormulas'][$columnTitle], $aCol) !== false) {
                                        if ($row[$aCol] == null)
                                            $nonNullValues--;
                                    }
                                }
                            } else if ($row[$columnTitle] == null){
                                    $nonNullValues--;
                            }
                        }
                ...
                if (!empty($minColumns)) {
                    ...
                    foreach ($output['minColumns'] as $key => $columnTitle) {
                        $pos = strpos($minColumns[$key], 'formula');
                        if($pos !== false){
                            ...
                            array_unshift($functionsParsedQuery['SELECT'], array(
                                    'expr_type' => 'colref',
                                    'base_expr' => 'MIN(' . $wdtParameters['columnFormulas'][$formulaColumnTitle] . ')
                                                 AS ' . $leftSysIdentifier . $columnTitle . $rightSysIdentifier,
                                    'delim' => ','
                                )
                            );
                ...
                if (!empty($maxColumns)) {
                    ...
                    foreach ($output['maxColumns'] as $key => $columnTitle) {
                        $pos = strpos($maxColumns[$key], 'formula');
                        if($pos !== false){
                            ...
                            array_unshift($functionsParsedQuery['SELECT'], array(
                                    'expr_type' => 'colref',
                                    'base_expr' => 'MAX(' . $wdtParameters['columnFormulas'][$formulaColumnTitle] . ')
                                                 AS ' . $leftSysIdentifier . $columnTitle . $rightSysIdentifier,
                                    'delim' => ','
                                )
                            );
